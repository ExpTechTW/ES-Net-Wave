...
class StationSelector {
  ...
  async initialize() {
    ...
    try {
      ...
      if (savedStationId) {
        ...
        if (
          window.wsService &&
          typeof window.wsService.setStation === "function"
        ) {
  ...
  createStationList() {
    ...
    const groupedStations =
      this.stationManager!.getESNetStationsGroupedByCity();
  ...
  toggleCity(city: string) {
    const cityStationsContainer = document.querySelector(
      `.city-stations[data-city="${city}"]`,
    );
    const cityHeader = document.querySelector(
      `.city-header[data-city="${city}"]`,
    );
    ...
    if (cityStationsContainer && cityHeader) {
      ...
      if (isExpanded) {
        ...
        const allExpandedCities = document.querySelectorAll(
          ".city-stations.expanded",
        );
        const allExpandedHeaders = document.querySelectorAll(
  ...
  setupEventHandlers() {
    ...
    document.addEventListener("click", (event) => {
      if (
        !stationCard.contains(event.target as Node) &&
        !stationSelection.contains(event.target as Node)
      ) {
  ...
  updateSelectedStationUI(stationId: string) {
    ...
    const selectedStationItem = document.querySelector(
      `.station-item[data-station-id="${stationId}"]`,
    );
