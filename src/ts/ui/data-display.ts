...
class DataDisplay {
  ...
  updateIntensityData(intensity: number, pga: number, timestamp: number) {
    ...
    const intensityLevelElement = document.getElementById(
      "val-int-level",
    ) as HTMLElement;
    ...
    const cardIntLevel = document.getElementById(
      "card-int-level",
    ) as HTMLElement;
    if (cardIntLevel) {
      const level = ES.getIntensityLevel(intensity);
  ...
  updateStationInfo(stationId: string) {
    ...
    if (stationElement) {
      if (window.stationSelector) {
        const stationInfo =
          window.stationSelector.stationManager.getStationInfo(stationId);
